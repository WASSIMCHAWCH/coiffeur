...
import { getWhatsAppLinkTo, getConfirmationMessageToClient } from '../utils/whatsapp';
import { formatDateFR } from '../utils/date';

/**
 * Popup de confirmation de rendez-vous avec bouton WhatsApp vers le client.
 *
 * Props :
 *  - appt    : objet rendez-vous { id, clientName, clientPhone, serviceName, date, startTime }
 *  - onClose : fermer le modal
 */
export default function ConfirmAppointmentModal({ appt, onClose }) {
  ...
  // Message WhatsApp en arabe tunisien avec lien de suivi
  const waMessage = getConfirmationMessageToClient({
  ...
  const waLink = getWhatsAppLinkTo(appt.clientPhone, waMessage);
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: '20px' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
            Le rendez-vous a été validé avec succès.
          </p>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
            ...
            <strong style={{ color: 'var(--success)' }}>{appt.startTime}</strong>
          </div>
        </div>
